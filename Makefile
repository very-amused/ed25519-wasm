...
all:
	emcc $(O) --no-entry -s EXPORTED_FUNCTIONS=$(EXPORTED_FUNCTIONS) -I $(INCLUDE) $(SRC) -o $(DESTDIR)/ed25519.wasm

debug:
	emcc $(DEBUG_FLAGS) --no-entry -s EXPORTED_FUNCTIONS=$(EXPORTED_FUNCTIONS) -I $(INCLUDE) $(SRC) -o $(DESTDIR)/ed25519.wasm
